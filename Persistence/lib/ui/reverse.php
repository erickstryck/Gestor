    ...
    <div id="tabs">
        ...
        <div id="principal">
            ...
            <p>
                ...
                <select name="options[tipo_geracao]" id="tipo_geracao">
                    ...
                    <option value="2"<?php selected(2, 'options', 'tipo_geracao'); ?>>Gerar arquivos em um ZIP na
                        class-path
                    </option>
                </select>
            </p>
            ...
            <p>
                ...
                <select name="options[classMapping]" id="classMapping">
                    ...
                    <option value="annotations"<?php selected('annotations', 'options', 'classMapping'); ?>>
                        Annotations
                    </option>
                </select>
            </p>
        ...
        <div id="personalizacao">
            <p>
                ...
                <input id="remove_prefix" name="options[remove_prefix]" type="text"
                       value="<?php post('options', 'remove_prefix'); ?>"/>
            </p>

            <p>
                <label for="remove_count_chars_start">Remover </label>
                <input style="width: 60px;" id="remove_count_chars_start" name="options[remove_count_chars_start]"
                       type="text" value="<?php post('options', 'remove_count_chars_start'); ?>"/>
                caracteres do inicio das tabelas </p>

            <p>
                <label for="remove_count_chars_end">Remover </label>
                <input style="width: 60px;" id="remove_count_chars_end" name="options[remove_count_chars_end]"
                       type="text" value="<?php post('options', 'remove_count_chars_end'); ?>"/>
                caracteres do final das tabelas </p>

            <p>
                <label for="format_classname">Formatar nome de classe </label>
                <input style="width: 60px;" id="format_classname" name="options[format_classname]" type="text"
                       value="<?php post('options', 'format_classname'); ?>"/>
                (utiliza sprintf para formatar o nome da classe) </p>

            <p>
                <label for="schema_name">Schema</label>
                <input id="schema_name" name="options[schema_name]" type="text"
                       value="<?php post('options', 'schema_name'); ?>"/>
            </p>

            <p>
                <label for="many_to_many_style">Formato para auto-identificar tabelas M-N:</label>
                <input id="many_to_many_style" name="options[many_to_many_style]" type="text"
                       value="<?php post('options', 'many_to_many_style'); ?>"/>
            </p>
            ...
            <p>
                ...
                <input id="generateAccessors" name="options[generateAccessors]" type="checkbox"
                       value="1" <?php checked(1, 'options', 'generateAccessors'); ?> />
            </p>

            <p>
                <label for="create_entities_for_many_to_many">Gerar entidades para tabelas M-N</label>
                <input id="create_entities_for_many_to_many" name="options[create_entities_for_many_to_many]"
                       type="checkbox" value="1" <?php checked(1, 'options', 'create_entities_for_many_to_many'); ?> />
            </p>

            <p>
                <label for="keep_foreign_column_name">Manter nomes das colunas nas chaves estrangeiras?</label>
                <input id="keep_foreign_column_name" name="options[keep_foreign_column_name]" type="checkbox"
                       value="1" <?php checked(1, 'options', 'keep_foreign_column_name'); ?> />
            </p>

            <p>
                <label for="camel_case">Usar CamelCase?</label>
                <input id="camel_case" name="options[camel_case]" type="checkbox"
                       value="1" <?php checked(1, 'options', 'camel_case'); ?> />
            </p>

            <p>
                <label for="usar_dicionario">Dicion&aacute;rio de plural?</label>
                <input id="usar_dicionario" name="options[usar_dicionario]" type="checkbox"
                       value="1" <?php checked(1, 'options', 'usar_dicionario'); ?> />
            </p>

            <p>
                <label for="create_paths">Deseja criar os diret&oacute;rios?</label>
                <input id="create_paths" name="options[create_paths]" type="checkbox"
                       value="1" <?php checked(1, 'options', 'create_paths'); ?> />
            </p>
        </div>
        <div id="integracao">
            <p>
                <label for="create_dtos">Criar DTO's?</label>
                <input id="create_dtos" name="options[create_dtos]" type="checkbox"
                       value="1" <?php checked(1, 'options', 'create_dtos'); ?> />
            </p>

            <p>
                <label for="auto_cast_dto">Auto-Cast para o DTO quando usar toObject/allToObject?</label>
                <input id="auto_cast_dto" name="options[auto_cast_dto]" type="checkbox"
                       value="1" <?php checked(1, 'options', 'auto_cast_dto'); ?> />
            </p>

            <p>
                <label for="dto_format">Formato do nome do DTO</label>
                <input name="options[dto_format]" type="text" id="dto_format"
                       value="<?php post('options', 'dto_format'); ?>"/>
            </p>
            ...
            <p>
                ...
                <input id="create_models" name="options[create_models]" type="checkbox"
                       value="1" <?php checked(1, 'options', 'create_models'); ?> />
            </p>

            <p>
                <label for="model_path">Pasta para as models</label>
                <input name="options[model_path]" type="text" id="model_path"
                       value="<?php post('options', 'model_path'); ?>"/>
            </p>

            <p>
                <label for="model_format">Formato de nomes para models</label>
                <input name="options[model_format]" type="text" id="model_format" size="10"
                       value="<?php post('options', 'model_format'); ?>"/>
            </p>

            <p>
                <label for="model_context">Criar arquivo de contexto para MVC?</label>
                <input id="model_context" name="options[model_context]" type="checkbox"
                       value="1" <?php checked(1, 'options', 'model_context'); ?> />
            </p>

            <p>
                <label for="model_context_path">Pasta grava&ccedil;&atilde;o do arquivo de contexto</label>
                <input name="options[model_context_path]" type="text" id="model_context_path" size="10"
                       value="<?php post('options', 'model_context_path'); ?>"/>
            </p>
        </div>
        <div id="addons">
            <p>Add-ons ser&atilde;o executados automaticamente ap&oacute;s o termindo da engenharia reversa.</p>

            <p>Caso a lista de add-ons n&atilde;o esteja sendo exibida, confirme se os caminhos est&atilde;o digitados
                corretamente.</p>

            <p>Lumine ir&aacute; procurar por uma pasta chamada &quot;add-ons&quot; dentro da pasta definida em sua
                class-path.</p>
            ...
            <p>Para atualizar a lista, <a href="#" onclick="$('#acao').val(''); $('form').submit();">clique aqui</a>.
            </p>
                ...
                <div id="addons-tabs">
                    <ul>
                        <?php foreach ($addons as $idx => $item): ?>
                            <li>
                                <input type="checkbox" value="<?php echo get_class($item); ?>"
                                       name="addons[]" <?php echo !empty($_POST['addons']) && in_array(get_class($item), $_POST['addons']) ? 'checked="checked"' : ''; ?> />
        ...
        <div id="tabelas">
            <div id="lista_tabelas" style="padding-left: 200px;">Ap&oacute;s preencher os dados, clique em "Recuperar
                tabelas"
            </div>
            <p>&nbsp;</p>
        ...
        <div id="importar">
            <p align="center" style="padding: 30px;">Utilize esta tela para importar as configura&ccedil;&otilde;es do
                seu arquivo lumine-conf.php, assim voc&ecirc; n&atilde;o precisar&aacute; indicar as informa&ccedil;&otilde;es
                j&aacute; definidas na primeira engenharia reversa.</p>
        ...
        <div id="logs">
            <div id="log_geracao"> Nesta aba voc&ecirc; poder&aacute; acompanhar o log de gera&ccedil;&atilde;o de suas
                classes.
            </div>
        </div>
    </div>
